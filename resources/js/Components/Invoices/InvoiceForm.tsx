...
interface InvoiceFormProps {
    ...
    setData: (key: keyof InvoiceFormData, value: any) => void;
...
export default function InvoiceForm({
    ...
    const calcLineSubtotal = (item: InvoiceItemForm) => {
        const quantity = Number(item.quantity) || 0;
        const unitPrice = Number(item.unit_price) || 0;
        return quantity * unitPrice;
    };

    const calcLineTax = (item: InvoiceItemForm) => {
                ...
                <form onSubmit={onSubmit} className="space-y-6">
                    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
                        ...
                        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                <label className="mb-2 block text-sm font-medium text-slate-700">عرض السعر</label>
                                <select
                                    value={data.quotation_id ?? ''}
                                    onChange={(e) => setData('quotation_id', e.target.value || null)}
                                    className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                                >
                                ...
                                {errors.quotation_id && <p className="mt-2 text-sm text-red-600">{errors.quotation_id}</p>}
                            </div>
                            ...
                            <div>
                                ...
                                />
                                {errors.invoice_date && <p className="mt-2 text-sm text-red-600">{errors.invoice_date}</p>}
                            </div>
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
                        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                ...
                                <p className="mt-1 text-sm text-slate-500">أضف البنود والخدمات والأسعار لكل بند.</p>
                            </div>
                            ...
                            {data.items.map((item, index) => (
                                <div key={index} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:p-5">
                                    <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                        <h3 className="text-base font-bold text-slate-700">البند #{index + 1}</h3>
                                    ...
                                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                        <div>
                                            ...
                                            {errors[`items.${index}.item_name`] && (
                                                <p className="mt-2 text-sm text-red-600">{errors[`items.${index}.item_name`]}</p>
                                            )}
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            {errors[`items.${index}.quantity`] && (
                                                <p className="mt-2 text-sm text-red-600">{errors[`items.${index}.quantity`]}</p>
                                            )}
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            {errors[`items.${index}.unit_price`] && (
                                                <p className="mt-2 text-sm text-red-600">{errors[`items.${index}.unit_price`]}</p>
                                            )}
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            {errors[`items.${index}.tax_percent`] && (
                                                <p className="mt-2 text-sm text-red-600">{errors[`items.${index}.tax_percent`]}</p>
                                            )}
                                        </div>
                                    </div>
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
                        <h2 className="mb-6 text-lg font-bold text-slate-800">ملخص الفاتورة</h2>

                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:p-5">
                                <p className="text-sm text-slate-500">الإجمالي قبل الضريبة</p>
                                <p className="mt-2 text-2xl font-bold text-slate-800">{subtotal.toFixed(2)}</p>
                            </div>

                            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:p-5">
                                <p className="text-sm text-slate-500">إجمالي الضريبة</p>
                                <p className="mt-2 text-2xl font-bold text-slate-800">{taxAmount.toFixed(2)}</p>
                            </div>
                            ...
                            <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4 sm:p-5">
                                ...
                                <p className="mt-2 text-2xl font-bold text-blue-700">{total.toFixed(2)}</p>
                            </div>
                        </div>
